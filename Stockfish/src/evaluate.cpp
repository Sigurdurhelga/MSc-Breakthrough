...
constexpr Score MobilityBonus[][32] = {
    ...
    {S(-39, -36), S(-21, -15), S(3, 8),     S(3, 18),    S(14, 34),
     S(22, 54),  // Queens
     S(28, 61),   S(41, 73),   S(43, 79),   S(48, 92),   S(56, 94),
     S(60, 104),  S(60, 113),  S(66, 120),  S(67, 123),  S(70, 126),
     S(71, 133),  S(73, 136),  S(79, 140),  S(88, 143),  S(88, 148),
     S(99, 166),  S(102, 170), S(102, 175), S(106, 184), S(109, 191),
     S(113, 206), S(116, 212)}};
...
constexpr Score ThreatByRook[PIECE_TYPE_NB] = {S(0, 0),   S(3, 44), S(38, 71),
                                               S(38, 61), S(0, 38), S(51, 38)};

// PassedRank[Rank] contains a bonus according to the rank of a passed pawn
constexpr Score PassedRank[RANK_NB] = {S(0, 0),    S(10, 28), S(17, 33),
                                       S(15, 41),  S(62, 72), S(168, 177),
                                       S(276, 260)};
...
void Evaluation<T>::initialize() {
  ...
  mobilityArea[Us] =
      ~(b | pos.pieces(Us, KING, QUEEN) | pe->pawn_attacks(Them));
...
Score Evaluation<T>::pieces() {
  ...
  for (Square s = *pl; s != SQ_NONE; s = *++pl) {
    ...
    if (pos.blockers_for_king(Us) & s) b &= LineBB[pos.square<KING>(Us)][s];
    ...
    if (Pt == BISHOP || Pt == KNIGHT) {
      ...
      if (shift<Down>(pos.pieces(PAWN)) & s) score += MinorBehindPawn;
      ...
      if (Pt == BISHOP && pos.is_chess960() &&
          (s == relative_square(Us, SQ_A1) ||
    ...
    if (Pt == ROOK) {
      ...
      if (file_bb(s) & pos.pieces(QUEEN)) score += RookOnQueenFile;
...
Score Evaluation<T>::threats() const {
  ...
  if (T) Trace::add(THREAT, Us, score);
...
Score Evaluation<T>::passed() const {
  ...
  if (T) Trace::add(PASSED, Us, score);
...
Score Evaluation<T>::space() const {
  if (pos.non_pawn_material() < SpaceThreshold) return SCORE_ZERO;
...
Score Evaluation<T>::initiative(Score score) const {
  ...
  int complexity = 9 * pe->passed_count() + 11 * pos.count<PAWN>() +
                   9 * outflanking + 21 * pawnsOnBothFlanks +
                   51 * !pos.non_pawn_material() - 43 * almostUnwinnable - 95;
  ...
  if (T) Trace::add(INITIATIVE, make_score(u, v));
...
ScaleFactor Evaluation<T>::scale_factor(Value eg) const {
  ...
  // If scale is not already specific, scale down the endgame via general
  // heuristics
  if (sf == SCALE_FACTOR_NORMAL) {
    if (pos.opposite_bishops() && pos.non_pawn_material() == 2 * BishopValueMg)
      sf = 22;
    else
      sf = std::min(sf, 36 + (pos.opposite_bishops() ? 2 : 7) *
...
Value Evaluation<T>::value() {
  ...
  v = mg_value(score) * int(me->game_phase()) +
      eg_value(score) * int(PHASE_MIDGAME - me->game_phase()) * sf /
          SCALE_FACTOR_NORMAL;
